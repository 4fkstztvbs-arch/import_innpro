...
- **Stav (2026-09-17): objavená natívna cesta v Shoptete, úplne mimo OAuth aplikácie.** Shoptet má vlastný natívny mechanizmus na dostanie Heureka recenzií do e-shopu, nezávislý od REST API prístupu spomenutého vyššie:
  - **Import do Shoptetu:** `Marketing → Hodnotenie → záložka Nastavenia` → tlačidlo na import až 500 posledných recenzií z Heureky priamo do natívnej záložky "Hodnotenia" na produkte. Vyžaduje Heureka tajný kľúč zadaný v `Prepojenie → Heureka` (kľúč sa získa v Heureka administrácii, `Overené zákazníkmi → Nastavenia a dáta z dotazníkov`) — **neoverené, či ide o ten istý kľúč** ako `HEUREKA_PRODUCT_REVIEW_KEY`/`export-product-review.php` použitý v `fetch-heureka-product-reviews.js` vyššie, Shoptetova dokumentácia to nešpecifikuje (treba overiť priamo v administrácii). Existuje aj samostatný marketplace doplnok len na tento účel (`doplnky.shoptet.sk/heureka-recenzie-a-hodnotenie`).
  - **Zobrazenie na produkte:** platený doplnok **Widget hodnotenia produktu** (`doplnky.shoptet.sk/widget-hodnotenia-produktu`, 4 €/mesiac alebo 46 €/rok, 30 dní skúšobne zadarmo) číta recenzie priamo z natívnej záložky "Hodnotenia" **vrátane tých importovaných z Heureky** a zobrazí ich na stránke produktu (slider s automatickým posúvaním alebo statické dlaždice, výber pozície pre desktop/tablet/mobil, farby/texty, minimálny počet hviezd). Nevyžaduje žiadne API ani OAuth aplikáciu — aktivuje sa priamo v administrácii Shoptetu.
  - **Dôsledok pre tento repozitár:** ak sa natívny import (prvý bod) zapne, cieľ "zobraziť Heureka recenzie v e-shope" sa dá splniť bez OAuth aplikácie a bez `data/heureka-reviews.json` — `fetch-heureka-product-reviews.js` a `.github/workflows/heureka-reviews.yml` by sa tým pre tento účel stali nadbytočné (mohli by mať zmysel ďalej len ak sa ich dáta využijú aj inak, napr. pri cenových/marketingových rozhodnutiach). Zámerne sa do skriptu/workflow nezasahuje, kým sa priamo v Shoptet administrácii nepotvrdí, že natívny import funguje a pokrýva to, čo treba — až potom má zmysel rozhodnúť o ich prípadnom zrušení.
