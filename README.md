...
**Dôležité zistenie (opravené):** pôvodne sa predpokladalo, že T01 import faktúry v Omege automaticky vygeneruje aj výdajku zo skladu. Reálnym testom sa ukázalo, že to **neplatí** — pri TXT importe sa do poľa "Kód položky" zapíše len text, ktorý sa síce v UI zhoduje s číslom karty (aj vyhľadávač karty ho správne nájde), no nevytvorí sa tým skutočná väzba na skladovú kartu akú Omega vytvorí pri ručnom výbere z katalógu — Omega sa preto pri uložení faktúry nepýta na výdajku. `transform-omega-invoices.js`/`omega-import.html` preto popri T01 (faktúra) generujú aj **samostatný T02 výdaj** (`omega-*-vydajka.txt`, typ `V`, záporné množstvo) pre všetky položky s rozpoznanou skladovou kartou, previazaný na číslo faktúry (stĺpec 18). Mapovanie stĺpcov (38/30) bolo odvodené z reálneho T02 exportu výdajky vytvorenej ručne v Omege. **Poradie importu: najprv T01 (faktúra), potom T02 (výdajka)** — výdajka sa v UI/CLI vygeneruje len ak faktúra obsahuje aspoň jednu položku so známou kartou.
