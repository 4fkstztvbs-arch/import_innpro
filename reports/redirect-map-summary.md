...
Vygenerované: 2026-08-09T19:21:04.635Z
...
Zo 4808 spárovaných produktov sa novú URL podarilo dohľadať pre **4557** (stav `OK`); zvyšných **251** má stav `CAKA_NA_NOVU_URL` — EAN sa v Heureka feede nového shopu nenašiel (typicky produkt momentálne nedostupný/mimo skladu, Shoptet takéto produkty do feedu nezahŕňa). Tie sa doplnia pri ďalšom spustení tohto skriptu, keď budú dané produkty skladom.

`redirect-map-shoptet-import.csv` obsahuje 4557 riadkov v tvare fromUrl;toUrl;automatic (relatívne cesty, lomítko na konci) — pripravené na priamy import do **Marketing → Základné SEO → Presmerovanie adries (URL)**. Cieľová cesta je len relatívna (bez domény `806405.myshoptet.com`), takže bude fungovať aj po prepnutí na finálnu doménu `premiumstore.sk`.
