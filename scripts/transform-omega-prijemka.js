...
// Excel import predajnych cien pre nove karty (Sklad -> Skladove karty -> Import z Excelu) -
// samostatny subor od T03, naimportuje sa AZ PO zalozeni kariet cez T03 (karty musia uz existovat).
// Nazvy stlpcov MUSIA byt presne takto (bez diakritiky, vratane cislovania "Cena1") - podla
// realneho dialogu "Podmienky pre import cennika" v Omege.
function buildNewCardPricesWorkbook(newCards) {
  const rows = newCards.map((r) => ({
    'Kod skladu': SKLAD_KOD,
    'Nazov karty': r.name,
    'Cislo karty': r.cardCode,
    'Cena1 Kod': PRICE_LEVEL_CODE,
    'Cena1 bez DPH': Number(resolveSellNet(r.unitPriceNet, r.feedPriceVat).toFixed(4)),
  }));
