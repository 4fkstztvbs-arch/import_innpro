...
function buildShopitemXml(p) {
  ...
  if ((isHeurekaHidden(p.defaultCategory, p.price) || cannotCompeteOnPrice(p.ean) || isCpcNonConverter(p.ean)) && !isPilotUnhidden(p.ean)) parts.push('<HEUREKA_HIDDEN>1</HEUREKA_HIDDEN>');
  if (p.images.length) {
    parts.push('<IMAGES>');
